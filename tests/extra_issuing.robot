...
Set Up Extra Issuing Logic Test Suite
    ...
    VAR    ${CLIENT_X25519_CERT}  ${X25519_cert}   scope=Global
    VAR    ${CLIENT_X448_CERT}  ${X448_cert}   scope=Global
    VAR    ${CLIENT_ECC_CERT}  ${ecc_cert}   scope=Global
    ${ecc_key}=   Load Private Key From File    data/keys/private-key-ecdsa.pem
    VAR    ${CLIENT_ECC_KEY}  ${ecc_key}   scope=Global
    ${x25519_key}=   Load Private Key From File    data/keys/private-key-x25519.pem
    VAR    ${CLIENT_X25519_KEY}  ${x25519_key}   scope=Global
    ${x448_key}=   Load Private Key From File    data/keys/private-key-x448.pem
    VAR    ${CLIENT_X448_KEY}  ${x448_key}   scope=Global

Build Encrypted Key Request
    [Documentation]    Build an request with an encrypted key as POPO.
    [Arguments]    ${key}   ${ca_cert}   ${cmp_protection}   ${ecc_key}=${None}
    ...            &{args}
...
CA Must Accept EncrCert POPO For Request With X25519 Key
    ...
    VAR    ${X25519_KEY}    ${new_key}    scope=GLOBAL
...
CA Must Accept EncrCert POPO For Request With X448 Key
    ...
    ${result}=    Should Contain    ${ALLOWED_ALGORITHM}    x448
    ${extensions}=    Prepare Extensions    key_usage=keyAgreement
    ...
    IF    not ${ALLOW_IMPLICIT_CONFIRM}
        ...
        PKIMessage Body Type Must Be    ${pki_conf}    pkiconf
    END
    VAR    ${X448_CERT}    ${cert}    scope=GLOBAL
    VAR    ${X448_CERT}    ${new_key}    scope=GLOBAL
...
CA Must Accept ChallengeResp POPO For Request With X25519 Key
   [Documentation]    According to RFC 4210bis-18 Section 5.2.8.3.3. the Client can use the `challengeResp`
    ...
    [Tags]    positive    challenge   popo  issuing  robot:skip-on-failure  advanced  x25519   challenge-response  envData
...
CA Must Accept ChallengeResp POPO For KeyEnc
    ...
    [Tags]    positive    challenge   popo  issuing  robot:skip-on-failure  advanced  keyEnc  challenge-response  envData
...
CA May Process The UTF8Pairs String
    [Documentation]    According to RFC 4211 section-7.1 can the Client use this control to convey text-based information
    ...                to the RA or CA issuing a certificate. We send some UTF8Pairs string inside the `CertRequest`.
    ...                The CA MUST accept the request and issue a certificate and may use the information to
    ...                issue a certificate.
    ...
    ${extensions}=  Prepare Extensions   ${key}   key_usage=keyAgreement,digitalSignature
...
CA MUST Used the CertRequest Inside the reqInfo
    ...
    ...                let the POPO be valid, it can use the reqInfo field to set the CertTemplate which should be issued.
    ...                We send a PKIMessage with a CertRequest inside the reqInfo. The CA MUST accept the request
    ...                and use the `reqInfo` CertRequest to issue the certificate.
    [Tags]    positive   reqInfo  issuing  advanced  robot:skip-on-failure
