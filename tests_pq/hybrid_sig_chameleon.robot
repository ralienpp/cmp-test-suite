...
CA MUST Issue a valid Chameleon Cert
    ...
    [Tags]    chameleon   hybrid-cert  positive
    ${pq_key}=  Generate Default PQ Key
    ...
    VAR   ${CHAMELEON_CERT}    ${cert}
    VAR   ${CHAMELEON_KEY}    ${trad_key}
    VAR   ${CHAMELEON_DELTA_CERT}    ${delta_cert}
    VAR   ${CHAMELEON_DELTA_KEY}    ${pq_key}
...
CA MUST Detect Invalid Secondary POP in Paired CSR
    ...
    [Tags]     chameleon   hybrid-cert  negative
    ${pq_key}=  Generate Default PQ Key
    ${trad_key}=  Generate Key   rsa   length=2048
...
CA SHALL NOT Include Extensions in the Base Certificate
    ...
    ...                `badCertTemplate`.
    [Tags]     chameleon   hybrid-cert  negative   robot:skip-on-failure
    ${pq_key}=  Generate Default PQ Key
...
CA SHOULD NOT Mark The DCD Extension as Critical
    ...
    [Tags]     chameleon   hybrid-cert  robot:skip-on-failure
    ${pq_key}=  Generate Default PQ Key
    ${trad_key}=  Generate Default Key
...
CA MUST Revoke a Delta Cert
    ...
    [Tags]    chameleon   hybrid-cert  positive  rr
...
CA MUST Check That The Delta Key Was Not Previously Revoked
    ...
    [Tags]     chameleon   hybrid-cert   hybrid-auth
