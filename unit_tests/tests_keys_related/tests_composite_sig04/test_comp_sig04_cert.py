...
class TestCompositeSig04Cert(unittest.TestCase):

    @classmethod
    def setUpClass(cls):
        cls.bad_rsa_key = generate_key("bad_rsa_key")
        cls.ml_dsa_44_key = generate_key("ml-dsa-44")
        cls.comp_bad_key = CompositeSig04PrivateKey(trad_key=cls.bad_rsa_key, pq_key=cls.ml_dsa_44_key)
    ...
    def test_cert_comp_sig_pure_rsa(self):
        ...
        comp_key = CompositeSig04PrivateKey(trad_key=rsa_key, pq_key=self.ml_dsa_44_key)
