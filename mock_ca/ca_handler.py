...
class CAHandler:
    ...
    def process_chameleon(self, pki_message: rfc9480.PKIMessage) -> rfc9480.PKIMessage:
        ...
        if pki_message["body"].getName() == "p10cr":
            ...
            raise NotImplementedError(
                "Not implemented to handle a chameleon request with body: {}".format(pki_message["body"].getName())
            )
...
def get_pubkey(serial_number):
    """Get the alternative public key, of a sun hybrid certificate."""
...
def get_signature(serial_number):
    """Get the alternative signature, of a sun hybrid certificate."""
    serial_number = int(serial_number)
